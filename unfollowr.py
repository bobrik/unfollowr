		...
		while True:
			...
			if followers != False:
				for i, user_id in enumerate(followers):
					...
					self.dbstore.save_unfollows(user_id, dict((id, named_user_unfollowers[id]) for id in named_user_unfollowers if remaining_unfollowers != False and remaining_unfollowers.count(id) == 0))
			else:
